@section('meta_tag')
    ...
    <title>Pengurus ICRP | {{ $management->name }}</title>
@endsection
...
<x-layouts.main>
    ...
    <section class="py-16 bg-gradient-to-b from-white to-gray-100">
        <div class="container mx-auto px-4">
            <!-- Tombol Back -->
            <div class="mb-8">
                <a href="{{ route('pengurus') }}" class="inline-flex items-center px-4 py-2 bg-gray-200 hover:bg-gray-300 rounded-full text-gray-800 transition duration-300 ease-in-out">
                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path>
                    </svg>
                    Kembali ke Jajaran Pengurus
                </a>
            </div>

            <!-- Management Header -->
            <div class="text-center mb-12">
                <h2 class="text-4xl font-bold text-gray-800 mb-2">{{ $management->name }}</h2>
                <div class="w-24 h-1 bg-primary-600 mx-auto mb-4"></div>
                <p class="text-xl text-gray-600">{{ $management->position }} | {{ $management->dewan }}</p>
            </div>
            ...
            <div class="flex flex-col lg:flex-row gap-10">
                ...
                <div class="lg:w-1/3">
                    <div class="bg-white rounded-lg shadow-xl overflow-hidden">
                        <div class="relative">
                            @if($management->image)
                                <img src="{{ Storage::url('managements/' . $management->image) }}" alt="{{ $management->name }}" class="w-full h-auto">
                            @else
                            ...
                            @endif
                            @if($management->nickname)
                                <div class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black to-transparent p-4">
                                    <h3 class="text-white text-2xl font-bold">{{ $management->nickname }}</h3>
                                </div>
                            @endif
                        </div>
                        ...
                        <div class="p-6 space-y-4">
                            <div class="grid grid-cols-2 gap-2 text-sm">
                                ...
                                <div>{{ $management->birth_date ? $management->birth_date->format('d F Y') : '-' }}</div>

                                @if($management->death_date)
                                    <div class="font-semibold text-gray-700">Wafat:</div>
                                    <div>{{ $management->death_date->format('d F Y') }}</div>
                                @endif

                                <div class="font-semibold text-gray-700">Tempat Lahir:</div>
                                <div>{{ $management->birth_place }}</div>

                                <div class="font-semibold text-gray-700">Dikenal sebagai:</div>
                                <div>{{ $management->known_as }}</div>
                            </div>
                ...
                <div class="lg:w-2/3">
                    <div class="bg-white rounded-lg shadow-lg p-8">
                        ...
                        @if($management->contributions->count() > 0)
                            <div class="mt-8">
                                ...
                                <div class="grid md:grid-cols-2 gap-4">
                                    @foreach($management->contributions as $contribution)
                        ...
                        @if($management->legacies->count() > 0)
                            <div class="mt-8">
                                <div class="flex items-center mb-6">
                                    <div class="w-12 h-12 bg-primary-600 rounded-full flex items-center justify-center text-white">
                                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                ...
                                <div class="prose max-w-none text-gray-700">
                                    {!! $management->legacies->first()->content !!}
                                </div>
                            </div>
                        @endif
                    </div>
