<div class="space-y-6">
    <div class="flex items-center justify-between">
        <flux:heading size="xl" level="2">Manage Founders</flux:heading>
        <flux:breadcrumbs>
            <flux:breadcrumbs.item href="{{ route('dashboard') }}">Dashboard</flux:breadcrumbs.item>
            <flux:breadcrumbs.item>Founders</flux:breadcrumbs.item>
        </flux:breadcrumbs>
    </div>

    <div class="flex items-center justify-between">
        <div class="w-1/3">
            <flux:input wire:model.live.debounce.300ms="search" placeholder="Search founders..." icon="magnifying-glass" />
        </div>
                        ...
                        <tr>
                            <td class="px-6 py-4 text-sm font-medium text-zinc-900">
                                <div class="flex items-center">
                                    @if ($founder->image && Storage::disk('public')->exists($founder->image))
                                        <img src="{{ Storage::url($founder->image) }}" alt="{{ $founder->name }}" class="w-10 h-10 object-cover rounded-full mr-3">
                                    @else
                                    ...
                                    <div>
                                        ...
                                        @if($founder->nickname)
                                            <div class="text-xs text-zinc-500">({{ $founder->nickname }})</div>
                                        @endif
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex justify-end space-x-2">
                                    <flux:button size="sm" icon="pencil" wire:click="edit({{ $founder->id }})" class="text-blue-800! bg-blue-400/20!" />
                                    <flux:button size="sm" icon="trash" wire:click="confirmDelete({{ $founder->id }})" class="text-red-800! bg-red-400/20!" />
                                </div>
                            </td>
                        </tr>
                    @empty
    ...
    <!-- Founder Form Modal -->
    <flux:modal name="founderModal" class="min-w-sm md:min-w-2xl lg:min-w-3xl">
        <div class="space-y-6">
            <flux:heading size="lg">{{ $founderId ? 'Edit Founder' : 'Add Founder' }}</flux:heading>
            ...
            <form wire:submit.prevent="save" class="space-y-6">
                ...
                <div x-data="{ activeTab: 'basic' }">
                    ...
                    <div x-show="activeTab === 'basic'" class="space-y-4">
                        ...
                        <flux:field>
                            ...
                            <flux:textarea id="quote" wire:model="quote" rows="2" placeholder="Notable quote from this founder" />
                        ...
                        <flux:field>
                            ...
                            @if ($image && !$newImage && Storage::disk('public')->exists($image))
                                <div class="mb-4">
                                    ...
                                    <img src="{{ Storage::url($image) }}" alt="Current founder photo" class="max-w-xs h-auto rounded-lg shadow-md">
                                </div>
                            @endif
                            ...
                            @if ($newImage)
                                <div class="mt-4">
                                    ...
                                    <img src="{{ $newImage->temporaryUrl() }}" alt="New founder photo" class="max-w-xs h-auto rounded-lg shadow-md">
                                </div>
                            @endif
                        </flux:field>
                    </div>
                ...
                <div class="flex justify-end gap-x-2 pt-4 border-t">
                    ...
                    <flux:button variant="primary" type="submit">{{ $founderId ? 'Update' : 'Create' }}</flux:button>
                </div>
            </form>
        </div>
    </flux:modal>
    ...
    <flux:modal name="deleteFounderModal" class="min-w-[22rem]">
        <div class="space-y-6">
